...
/**
 * Single blog post component. Return a container with post details and comments,
 * with edit button if user is post author and comment form if logged in.
 *
 *
  ...
  useEffect(() => {
    fetch(`/api/blog/${match.params.slug}`)
      ...
      .then((res) => {
        setPostDetail(res);
        setDateString(
          dateBuilder(res["author"], res["created_on"], res["updated_on"])
        );
        setTagList(
          res["tags"].map((tag) => (
            <Badge key={tag} className="list-badge badge badge-primary">
              {tag}
            </Badge>
          ))
        );
  ...
  return (
    <Container className="pt-5">
      <Col lg={12}>
        ...
        <Row>
          <Col>
            <p className="text-muted">Tags: {tagList}</p>
          </Col>
        </Row>
        <Row>
          <Col>
            <div
              ...
              dangerouslySetInnerHTML={{
                __html: DOMPurify.sanitize(postDetail["body"]),
              }}
            />
          </Col>
        </Row>
        <Row className="comments">
          <Col>
            <CommentList
              ...
              postedNewComment={postedNewComment}
            />
          </Col>
        </Row>
        <Row>
